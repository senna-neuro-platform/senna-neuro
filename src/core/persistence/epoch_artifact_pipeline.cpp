...
EpochArtifactPipeline::EpochArtifactPipeline(EpochArtifactConfig config)
    : config_(std::move(config)), experiment_writer_(config_.experiment_file.string()) {
    ensure_directories();
}

void EpochArtifactPipeline::persist_epoch(
    const std::size_t epoch, const std::vector<senna::core::domain::SpikeEvent>& spike_trace,
...
void EpochArtifactPipeline::persist_epoch(
    ...
    const std::unordered_map<std::string, double>& metrics, const SimulationState* state) const {
...
std::filesystem::path EpochArtifactPipeline::outbox_file_path(const std::size_t epoch) const {
    char file_name[32]{};
    std::snprintf(file_name, sizeof(file_name), "epoch_%09zu.h5", epoch);
    return config_.outbox_dir / file_name;
}

void EpochArtifactPipeline::ensure_directories() const {
    const auto experiment_parent = config_.experiment_file.parent_path();
    if (!experiment_parent.empty()) {
        static_cast<void>(std::filesystem::create_directories(experiment_parent));
    }

    if (config_.outbox_dir.empty()) {
        throw std::invalid_argument("EpochArtifactConfig.outbox_dir must not be empty");
    }
    static_cast<void>(std::filesystem::create_directories(config_.outbox_dir));
}

void EpochArtifactPipeline::write_outbox_epoch_file(
    const std::size_t epoch, const std::vector<senna::core::domain::SpikeEvent>& spike_trace,
    const std::vector<senna::core::domain::Neuron>& neurons,
    const senna::core::domain::SynapseStore& synapses, const std::vector<MetricPoint>& metrics,
    const SimulationState* state) const {
