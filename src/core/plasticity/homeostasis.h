...
class Homeostasis final {
   public:
    explicit Homeostasis(const HomeostasisConfig config = {}) : config_(config) {
        validate_config();
    }
    ...
    void on_tick(std::vector<senna::core::domain::Neuron>& neurons,
        ...
        for (std::size_t neuron_index = 0U; neuron_index < neurons.size(); ++neuron_index) {
            ...
            const auto theta_delta =
                config_.eta_homeo * (neuron.average_rate() - config_.r_target_hz);
   ...
   private:
    void ensure_capacity(const std::size_t size) {
