...
MetricsCollector::MetricsCollector(const std::vector<senna::core::domain::Neuron>& neurons,
    ...
    for (const auto& neuron : neurons_) {
        if (neuron.type() == senna::core::domain::NeuronType::Excitatory) {
            ++excitatory_neuron_count_;
        } else {
            ++inhibitory_neuron_count_;
        }
    }
}
...
void MetricsCollector::on_spike(const senna::core::domain::SpikeEvent& spike) {
    ...
    if (index < neurons_.size()) {
        if (neurons_[index].type() == senna::core::domain::NeuronType::Excitatory) {
            ++excitatory_spikes_this_tick_;
        } else {
            ++inhibitory_spikes_this_tick_;
        }
    }
...
void MetricsCollector::on_tick(const senna::core::domain::Time t_start,
    ...
    const auto [e_rate_hz, i_rate_hz] = rates_by_type(snapshot_.tick_duration_seconds);
...
std::pair<double, double> MetricsCollector::rates_by_type(
    const double tick_duration_seconds) const noexcept {
    if (tick_duration_seconds <= 0.0) {
        return {0.0, 0.0};
    }

    const auto e_rate =
        excitatory_neuron_count_ == 0U
            ? 0.0
            : static_cast<double>(excitatory_spikes_this_tick_) /
                  (static_cast<double>(excitatory_neuron_count_) * tick_duration_seconds);
    const auto i_rate =
        inhibitory_neuron_count_ == 0U
            ? 0.0
            : static_cast<double>(inhibitory_spikes_this_tick_) /
                  (static_cast<double>(inhibitory_neuron_count_) * tick_duration_seconds);
    return {e_rate, i_rate};
}

void MetricsCollector::reset_tick_accumulators() noexcept {
    spikes_this_tick_ = 0U;
    excitatory_spikes_this_tick_ = 0U;
    inhibitory_spikes_this_tick_ = 0U;
