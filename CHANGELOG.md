...
- In [src/bindings/python_module.cpp](src/bindings/python_module.cpp), Winner-Take-All lateral inhibition is now applied in real time during simulation: when an output neuron spikes, it immediately injects inhibitory events into all other output neurons via the spike observer, instead of applying WTA only after the prediction is finalized. This enables proper competitive dynamics between output neurons during the 100-tick processing window.
