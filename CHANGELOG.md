...
- In [src/bindings/python_module.cpp](src/bindings/python_module.cpp), the output decoding strategy was switched from first-spike latency to spike-count: `decode_by_spike_count()` counts spikes per output neuron over the full simulation window and selects the neuron with the highest count. First-spike decoding was too fragile with random initial weights — whichever neuron happened to spike first won regardless of input, and aggressive WTA (-6.0) locked in that random winner. Spike-count decoding aggregates evidence over the entire 100-tick window, making predictions more robust and giving supervised weight updates a meaningful gradient to follow. Real-time WTA was removed as it worsened accuracy by suppressing correct neurons before they could accumulate spikes.
- In [src/bindings/python_module.cpp](src/bindings/python_module.cpp), wired STDP, homeostasis, and structural plasticity into the training pipeline. Previously these subsystems were fully implemented and tested in isolation but never instantiated or called during training — synapses in the processing volume kept random initial weights, preventing unsupervised feature learning. Now: STDP updates synaptic weights on every spike based on pre/post timing correlations; homeostasis adjusts neuron thresholds each tick to maintain target firing rates; structural plasticity periodically prunes weak synapses and sprouts new ones for quiet neurons. All three are disabled during eval mode. STDP traces are reset between samples to prevent cross-sample timing contamination.
...
- In [python/train.py](python/train.py), `exporter_snapshot_payload()` now includes `mean_active_neurons_ratio`, `mean_spikes_per_tick`, `ticks_total`, and `spikes_total` in the JSON snapshot written for the Prometheus exporter.
- In [infra/simulator/simulator_server.py](infra/simulator/simulator_server.py) and [infra/simulator/test_simulator_server.py](infra/simulator/test_simulator_server.py), the exporter dataclass, loader, and renderer were extended to read and publish the five new metrics as Prometheus gauges/counters, with full test coverage.
- In [configs/grafana/dashboards/senna-network-health.json](configs/grafana/dashboards/senna-network-health.json), a new Network Health dashboard was added with six panels: Active Neurons Current/Mean/Peak, Spikes Per Tick Current/Mean, Cumulative Spikes, Cumulative Ticks, Training Throughput, and Synapse Topology (synapse count with net structural change overlay).
- In [visualizer/index.html](visualizer/index.html), neuron type base colors were made brighter and more distinct (excitatory cyan, inhibitory magenta-pink, output gold), spike flashes now use per-type colored flashes instead of uniform white, and a glow layer matches each type's flash color for clear visual separation of spike wave origins.
- In [visualizer/index.html](visualizer/index.html), the control panel now includes color pickers for both neuron base colors and spike flash colors per type (Excitatory, Inhibitory, Output), allowing real-time customization of the visualization palette.
...
- In [visualizer/index.html](visualizer/index.html), the scene was reworked for stronger contrast: darker background/fog, brighter type colors, stronger lighting, a dedicated additive glow layer for active neurons, slower flash decay, and lattice bounds/layer guides so spikes and wavefronts are visually separable from the background.
- In [README.md](README.md), the exported metrics list was updated to include the new training progress and ETA gauges.
- In [src/core/metrics/metrics_collector.h](src/core/metrics/metrics_collector.h), [src/core/metrics/metrics_collector.cpp](src/core/metrics/metrics_collector.cpp), and [tests/test_metrics.cpp](tests/test_metrics.cpp), `senna_e_rate_hz`, `senna_i_rate_hz`, and `senna_ei_balance` were fixed to derive from real excitatory/inhibitory spike counts per tick instead of `Neuron::average_rate()`, which was not updated in the current runtime path and kept the Grafana `E/I Rates` and `E/I Balance` panels pinned to zero.
